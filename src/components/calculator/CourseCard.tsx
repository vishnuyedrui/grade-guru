...
export function CourseCard({
  ...
  const getGradeOptions = (name: string) =>
    name === "Learning Engagement"
      ? LE_GRADE_OPTIONS
      : SESSIONAL_GRADE_OPTIONS;

  /* ---------------- RECALCULATE ---------------- */

  const recalculateCourse = (assessments: Assessment[]) => {
    const sessionalPass = checkSessionalPassByMarks(assessments);

    const hasSpecial = assessments.some(a =>
      a.gradeLabel === "I" ||
      a.gradeLabel === "P" ||
      a.gradeLabel === "Ab/R"
    );
    ...
    // ✅ PASS for I / P / Ab-R (any mix)
    if (hasSpecial && sessionalPass === true) {
      const baseGP = 4;
      const effectiveGP =
        course.hasLab && course.labMarks !== null
          ? calculateFinalGradePointWithLab(baseGP, course.labMarks)
    ...
    // 🔹 NORMAL FLOW
    const rawWGP = calculateWGP(assessments);
    if (rawWGP === null) return;

    const wgp = Math.min(10, Math.ceil(rawWGP));
    let finalGP = wgp;

    if (course.hasLab && course.labMarks !== null) {
      finalGP = calculateFinalGradePointWithLab(wgp, course.labMarks);
    }
  ...
  const updateAssessmentGrade = (i: number, gradeLabel: string) => {
    const selected = getGradeOptions(course.assessments[i].name)
      .find(g => g.label === gradeLabel);
    ...
    const updated = course.assessments.map((a, idx) =>
      idx === i
        ? {
            ...a,
            gradeLabel,
            gradePoint: selected && selected.value >= 0 ? selected.value : null,
            marks: a.marks ?? null,
          }
        : a
    );

    recalculateCourse(updated);
  };

  /* ---------------- UPDATE MARKS ---------------- */

  const updateAssessmentMarks = (i: number, value: string) => {
    const marks =
  ...
  return (
    <Card className={cn("border-2", course.wgp !== null && "border-accent/30")}>
      <CardHeader>
                  ...
                  <tr key={a.name}>
                    ...
                    <td className="p-2">
                      <select
                        value={a.gradeLabel ?? ""}
                        onChange={e =>
                          updateAssessmentGrade(i, e.target.value)
                        }
                        className="border p-1 w-full"
                      >
                        <option value="">Select</option>
                        {getGradeOptions(a.name).map(g => (
                      ...
                      {needsMarks && isSessional && (
                        <Input
                          ...
                          value={a.marks ?? ""}
                          onChange={e =>
                            updateAssessmentMarks(i, e.target.value)
                          }
                        />
                      )}
                    </td>
